  ...
  return order.map((category) => {
    ...
    for (const d of items) {
      if (d.image?.trim() && !imageMap.has(d.image)) {
        ...
        imageNameMap.set(d.image, d.name);
      }
    }
    if (imageMap.size === 0) {
      imageMap.set("/indisponible.png", "center top");
      imageNameMap.set("/indisponible.png", category);
    }
    return { category, items, imageMap, imageNameMap };
  });
})();
...
const ImageCarousel = memo(function ImageCarousel({
  ...
  const currentSrc = entries[current]?.[0];
  const currentDishName = currentSrc ? (imageNameMap.get(currentSrc) ?? "") : "";
  ...
  return (
    <div className="relative overflow-hidden" style={{ border: "1px solid rgba(201,168,76,0.12)", height: `${height}px` }}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(to top, rgba(10,10,10,0.7) 0%, rgba(10,10,10,0.1) 45%, transparent 100%)" }}/>
...
const CategorySection = memo(function CategorySection({ category, items, imageMap, imageNameMap, index,}: CategoryData & { index: number }) {
  ...
  return (
    <div ref={ref} id={category} className="cat-section" style={{ scrollMarginTop: "130px" }}>
      ...
      <div className="sm:hidden">
        <div className="mb-6">
          <ImageCarousel imageMap={imageMap} imageNameMap={imageNameMap} category={category} height={224} />
        </div>
        <div>
      ...
      <div className="hidden sm:grid sm:grid-cols-2 gap-10 md:gap-16 items-start">
        <div className="sticky top-[130px]">
          <ImageCarousel imageMap={imageMap} imageNameMap={imageNameMap} category={category} height={420} />
    ...
    <>
      ...
      <div className="min-h-screen relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 py-16 sm:py-28 space-y-24 sm:space-y-36">
          {dishesByCategory.map(({ category, items, imageMap, imageNameMap }, i) =>
            items.length > 0 && (
              <CategorySection
                key={category}
                category={category}
